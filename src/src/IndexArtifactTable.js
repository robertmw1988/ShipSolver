...
function runBulkSearchesAndOutput() {
  ...
  var capacityIdx = paramHeaders.indexOf(CFG.paramsCols.capacity);
  var criteriaSets = [];
  var capacityMap = {}; // Map to store capacity by ship+duration key
  for (var i = 1; i < paramData.length; i++) {
    ...
    if (row[shipTypeIdx] || row[shipDurationIdx] || 
      ...
      // Store capacity for this ship+duration combination
      var rawShip = row[shipTypeIdx];
      var rawDuration = row[shipDurationIdx];
      var normalizedShip = _normalizeWithAliases('shipType', rawShip, aliases);
      var normalizedDuration = _normalizeWithAliases('shipDurationType', rawDuration, aliases);
      var capacityValue = Number(row[capacityIdx]);
      if (isNaN(capacityValue)) capacityValue = 0;

      var rawKey = (rawShip || '') + '|' + (rawDuration || '');
      if (rawKey !== '|') capacityMap[rawKey] = capacityValue;

      var normalizedKey = normalizedShip + '|' + normalizedDuration;
      if (normalizedKey !== '|') capacityMap[normalizedKey] = capacityValue;
    }
  }
  ...
  criteriaSets.forEach(function(criteria) {
    ...
    var matchingRows = filterDataByCriteria(allRowsAsObjects, criteria, aliases);
  ...
  if (combinedResults.length > 0) {
    ...
    // Transform results: calculate ratios and apply capacity
    var transformedResults = [];
    for (var i = 0; i < combinedResults.length; i++) {
      var row = combinedResults[i];
      // Build key to lookup capacity
      var shipType = row[CFG.dataCols.shipType] || row['Ship type'];
      var shipDuration = row[CFG.dataCols.shipDurationType] || row['Ship duration type'];
      var normalizedShipType = _normalizeWithAliases('shipType', shipType, aliases);
      var normalizedDurationType = _normalizeWithAliases('shipDurationType', shipDuration, aliases);
      var rawKey = (shipType || '') + '|' + (shipDuration || '');
      var normalizedKey = normalizedShipType + '|' + normalizedDurationType;
      var capacity = 0;
      if (capacityMap.hasOwnProperty(normalizedKey)) capacity = capacityMap[normalizedKey];
      else if (capacityMap.hasOwnProperty(rawKey)) capacity = capacityMap[rawKey];
      
      // Apply transformation
      var transformed = _transformToDropRates(headers, row, capacity);
      transformedResults.push(transformed);
    }
    
    // Convert results to 2D array
    var dataForSheet = convertObjectsTo2DArray(headers, transformedResults);
    ...
    Logger.log("Wrote %s unique rows to the Results sheet with drop rate calculations.", transformedResults.length);
  } else {
    resultsSheet.clearContents();
...
function filterDataByCriteria(dataObjectArray, criteria, aliases) {
  ...
  var columnMap = {};
  columnMap[CFG.paramsCols.shipType] = CFG.dataCols.shipType;
  columnMap[CFG.paramsCols.shipDurationType] = CFG.dataCols.shipDurationType;
  columnMap[CFG.paramsCols.level] = CFG.dataCols.missionLevel;

  aliases = aliases || _buildAliasesFromSheet();
  var normalizedCriteria = {};
  var normalizationKinds = {};
  var missionLevelKeyLower = String(CFG.dataCols.missionLevel).toLowerCase();
  ...
  for (var key in criteria) {
    ...
    var dataKeyLower = String(dataKey).toLowerCase();

    if (dataKey === CFG.dataCols.shipType) {
      normalizationKinds[dataKey] = 'shipType';
      value = _normalizeWithAliases('shipType', value, aliases);
    } else if (dataKey === CFG.dataCols.shipDurationType) {
      normalizationKinds[dataKey] = 'shipDurationType';
      value = _normalizeWithAliases('shipDurationType', value, aliases);
    } else if (dataKeyLower === missionLevelKeyLower) {
      value = Number(value);
    }
  ...
  if (!Object.keys(normalizedCriteria).length) return [];
  ...
  return dataObjectArray.filter(function(row) {
    for (var key in normalizedCriteria) {
      ...
      var targetValue = normalizedCriteria[key];
      var rowValue = _getRowValueCaseInsensitive(row, key);

      if (rowValue == null || rowValue === '') {
        return false;
      }

      if (normalizationKinds[key]) {
        rowValue = _normalizeWithAliases(normalizationKinds[key], rowValue, aliases);
        if (rowValue != targetValue) return false;
      } else if (String(key).toLowerCase() === missionLevelKeyLower) {
        rowValue = Number(rowValue);
        if (Number(rowValue) != Number(targetValue)) return false;
      } else {
        if (rowValue != targetValue) return false;
      }
    }
...
function _getRowValueCaseInsensitive(row, key) {
  if (row.hasOwnProperty(key)) return row[key];
  var target = String(key).toLowerCase();
  for (var prop in row) {
    if (row.hasOwnProperty(prop) && String(prop).toLowerCase() === target) {
      return row[prop];
    }
  }
  return row[key];
...
/**
 * Calculate the sum of artifact columns for a row.
 * Artifact columns start at index 4 (after ship type, duration, level, target).
 */
function _calculateArtifactSum(headers, rowObj) {
  var sum = 0;
  // Start from index 4 to skip the key columns
...
/**
 * Transform artifact values to average drop rates.
 * For each artifact column: (value / rowSum) * capacity
 */
function _transformToDropRates(headers, rowObj, capacity) {
  var rowSum = _calculateArtifactSum(headers, rowObj);
  if (rowSum === 0 || !capacity || capacity === 0) {
    return rowObj; // Return unchanged if sum is 0 or no capacity
  }
  ...
  for (var i = 0; i < headers.length; i++) {
    ...
    if (i < 4) {
      // Keep key columns unchanged
      transformed[header] = rowObj[header];
    } else {
      // Transform artifact columns
      var val = rowObj[header];
      if (typeof val === 'number' && !isNaN(val)) {
        transformed[header] = (val / rowSum) * capacity;
      } else {
        transformed[header] = val;
      }
    }
  }
  return transformed;
...
function GET_MISSION_ROWS_BY_PARAM() {
  ...
  for (var s = 0; s < stKeys.length; s++) {
    ...
    for (var d = 0; d < duKeys.length; d++) {
      ...
      var paramInfo = paramMap[st + '␞' + du];
      Logger.log('Processing %s - %s: param info = %s', st, du, JSON.stringify(paramInfo));
      var pLv = (paramInfo == null || paramInfo.level == null || paramInfo.level === '') 
        ? CFG.defaultLevelIfMissing 
        : paramInfo.level;
...
function DIAG_PARAM_FOR_PAIR(ship, duration) {
  ...
  Logger.log('  Ship: %s (from "%s")', st, ship);
  Logger.log('  Duration: %s (from "%s")', du, duration);
  var paramInfo = params[key];
  Logger.log('  Configured level: %s', paramInfo ? paramInfo.level : 'not found');
  Logger.log('  Configured capacity: %s', paramInfo ? paramInfo.capacity : 'not found');
}
...
function DIAG_SAMPLE() {
  ...
  for (var k in param) {
    if (param.hasOwnProperty(k)) {
      some.push(k + ' -> level: ' + param[k].level + ', capacity: ' + param[k].capacity);
      if (++count >= 10) break;
    }
  }
...
function DIAG_PARAM_INDEX_MISMATCHES() {
  ...
  var params = _buildParamLevelMap();            // key: 'SHIP␞DURATION' -> { level, capacity }
